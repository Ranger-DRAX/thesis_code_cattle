...
==================================
Tune disease model and severity model separately on fold-0
Test: backbone_lr {5e-5, 1e-4} × head_lr {5e-4, 1e-3}
...
def hyperparameter_tuning():
    """
    Hyperparameter tuning for Option B on fold-0
    """
    ...
    print(f"{'='*80}")
    print(f"Testing {len(backbone_lrs) * len(head_lrs)} configurations on fold-0")
    ...
    for backbone_lr, head_lr in product(backbone_lrs, head_lrs):
        ...
        print(f"\n{'='*80}")
        print(f"Configuration {config_num}/{len(backbone_lrs)*len(head_lrs)}")
        print(f"Backbone LR: {backbone_lr}, Head LR: {head_lr}")
        print(f"{'='*80}")
        
        # Update configuration
        config = base_config.copy()
        config['disease_backbone_lr'] = backbone_lr
        config['disease_head_lr'] = head_lr
        config['severity_backbone_lr'] = backbone_lr
        config['severity_head_lr'] = head_lr
        
        # Create save directory
        config_name = f"backbone_{backbone_lr}_head_{head_lr}"
