...
====================================
Train both disease and severity models on all 5 folds with best hyperparameters
"""
...
# Add Option-B module to path
sys.path.insert(0, str(Path(__file__).parent))

from option_b import train_option_b, RESULTS_DIR
...
def run_5fold_cv():
    ...
    config = {
        ...
        'warmup_epochs': 5,
        'max_epochs': 25,
        'disease_backbone_lr': best_hp['backbone_lr'],
        'disease_head_lr': best_hp['head_lr'],
        'severity_backbone_lr': best_hp['backbone_lr'],
        'severity_head_lr': best_hp['head_lr'],
        'weight_decay': 1e-4,
        'dropout': 0.25,
        'patience': 5
    }
    ...
    print(f"\n{'='*80}")
    print("OPTION B - 5-FOLD CROSS-VALIDATION")
    print(f"{'='*80}\n")
    ...
    for fold_idx in range(5):
        ...
        try:
            ...
            fold_result = {
                ...
                'disease_precision': results['disease']['precision'],
                'disease_recall': results['disease']['recall'],
                'severity_accuracy': results['severity']['accuracy'],
                'severity_f1': results['severity']['f1_macro'],
                'severity_precision': results['severity']['precision'],
                'severity_recall': results['severity']['recall'],
                'hierarchical_accuracy': results['hierarchical_accuracy'],
                'disease_best_epoch': results['training']['disease_model']['best_epoch'],
                'severity_best_epoch': results['training']['severity_model']['best_epoch'],
    ...
    if len(valid_results) > 0:
        metrics = [
            'disease_accuracy', 'disease_f1', 'disease_precision', 'disease_recall',
            'severity_accuracy', 'severity_f1', 'severity_precision', 'severity_recall',
            'hierarchical_accuracy', 'disease_best_epoch', 'severity_best_epoch',
        ...
        print(f"  Precision: {stats['disease_precision']['mean']:.4f} ± {stats['disease_precision']['std']:.4f}")
        print(f"  Recall:    {stats['disease_recall']['mean']:.4f} ± {stats['disease_recall']['std']:.4f}")
        ...
        print(f"  Precision: {stats['severity_precision']['mean']:.4f} ± {stats['severity_precision']['std']:.4f}")
        print(f"  Recall:    {stats['severity_recall']['mean']:.4f} ± {stats['severity_recall']['std']:.4f}")
        
        print(f"\nHierarchical Accuracy: {stats['hierarchical_accuracy']['mean']:.4f} ± {stats['hierarchical_accuracy']['std']:.4f}")
        
        print("\nTraining Statistics:")
        print(f"  Disease best epoch: {stats['disease_best_epoch']['mean']:.1f} ± {stats['disease_best_epoch']['std']:.1f}")
        print(f"  Severity best epoch: {stats['severity_best_epoch']['mean']:.1f} ± {stats['severity_best_epoch']['std']:.1f}")
